...
export function ResponsePanel() {
    const { response, logs, tests, error, sending } = useRequestEditor();
    const wrapResponse = useUiPrefs((s) => s.wrapResponse);
    const [tab, setTab] = useState("body");
    // Initial body view follows the Appearance preference; the per-response Tree/
    // Raw toggle still overrides it for the session.
    const [raw, setRaw] = useState(() => useUiPrefs.getState().responseView === "raw");
    ...
    return (
        <div className="flex h-full flex-col" {...tourAnchor(ANCHORS.responsePanel)}>
            {/* status strip */}
            ...
            <div className="min-h-0 flex-1 overflow-auto">
                {tab === "body" &&
                    (hasJson && !raw ? (
                        <JsonView data={response.json!} />
                    ) : (
                        <pre
                            className={cn(
                                "mono p-4 text-[13px] text-fg",
                                wrapResponse ? "whitespace-pre-wrap break-all" : "whitespace-pre",
                            )}
                        >
                ...
                {tab === "console" && (
                    <div className="p-3">
                        {logs.length === 0 ? (
                            ...
                            <div className="mono space-y-0.5 text-[13px]">
                                {logs.map((line, i) => (
                                    <div
                                        key={i}
                                        className={cn(
                ...
                {tab === "tests" && (
                    <div className="space-y-1.5 p-3">
                        {tests.length === 0 ? (
                            <p className="px-1 text-[13px] text-subtle">
                                No assertions ran. Add <span className="text-fg">assert</span> blocks (or use{" "}
                                <code className="mono">pm.test()</code>) in Post-response scripts.
                            </p>
                        ) : (
                            tests.map((t, i) => (
                                <div
